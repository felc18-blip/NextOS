...
post_makeinstall_target() {
  # Install DBus policy allowing root to own org.pulseaudio.Server (PipeWire
  # pulse compat shim). Without it, pipewire-pulse logs "Failed to acquire" on
  # every boot. Idempotent: only install if file exists in package config.
  if [ -f "${PKG_DIR}/config/dbus-1/system.d/pipewire-pulse.conf" ]; then
    mkdir -p ${INSTALL}/etc/dbus-1/system.d
    cp ${PKG_DIR}/config/dbus-1/system.d/pipewire-pulse.conf ${INSTALL}/etc/dbus-1/system.d/
  fi
