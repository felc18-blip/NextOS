...
make_target() {
  unset DISTRO
  # mame2016 source de 2016 — GCC 16 trata muitos warnings legacy como error.
  # Best-effort em aarch64 (mame2016 pesado pra Mali-450 mesmo).
  ...
  make ${PKG_MAKE_OPTS_TARGET} OVERRIDE_CC=${CC} OVERRIDE_CXX=${CXX} OVERRIDE_LD=$LD AR=${AR} ${MAKEFLAGS} || true
}

makeinstall_target() {
  mkdir -p ${INSTALL}/usr/lib/libretro
  if compgen -G "*.so" > /dev/null 2>&1; then
    cp *.so ${INSTALL}/usr/lib/libretro/mame2016_libretro.so
    mkdir -p ${INSTALL}/usr/config/retroarch/savefiles/mame2016/hi
    [ -f metadata/hiscore.dat ] && cp metadata/hiscore.dat ${INSTALL}/usr/config/retroarch/savefiles/mame2016/hi
  else
    echo "mame2016_libretro.so not built — skipped"
  fi
}
