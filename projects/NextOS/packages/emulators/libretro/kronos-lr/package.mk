...
#  make -C ${PKG_BUILD}/yabause/src/libretro/ generate-files
  ...
  if [ "${TARGET_ARCH}" = "aarch64" ]; then
    ...
    # GLSL ES (Mali Valhall G310) exige precision default pra TODOS tipos em
    # compute shaders. vdp1_start_end_base so declarava 'precision highp float'.
    # outSurface (image2D) aborta com 'S0032: no default precision defined'.
    # Pattern \\n no arquivo C string = 2 bytes (backslash + n), sed precisa
    # \\n no pattern em single-quote shell.
    # Adicionar precision int+image2D+sampler2D APOS o "precision highp float\n":
    for SH in \
      ${PKG_BUILD}/yabause/src/core/video/opengl/compute_shader/include/vdp1_prog_compute.h \
      ${PKG_BUILD}/yabause/src/core/video/opengl/compute_shader/include/vdp1_prog_compute_upscale.h \
      ${PKG_BUILD}/yabause/src/core/video/opengl/common/src/rbg_compute.cpp; do
      [ -f "$SH" ] || continue
      sed -i 's|"precision highp float;\\n"|"precision highp float;\\nprecision highp int;\\nprecision highp image2D;\\nprecision highp sampler2D;\\n"|g' "$SH"
      # Adicional: highp inline em CADA declaracao 'uniform image2D' (Mali
      # nao respeitou o default em alguns drivers — inline forca).
      sed -i 's|writeonly uniform image2D|writeonly uniform highp image2D|g; s|readonly uniform image2D|readonly uniform highp image2D|g; s|\\(layout([^)]*)\\) uniform image2D|\\1 uniform highp image2D|g' "$SH"
    done
