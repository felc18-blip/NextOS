...
make_target() {
  # duckstation-lr é pesado (Vulkan-leaning); best-effort em aarch64 Mali-450.
  cd ${PKG_BUILD}/.${TARGET_NAME} && make ${MAKEFLAGS} || true
}

makeinstall_target() {
  mkdir -p ${INSTALL}/usr/lib/libretro
  if [ -f ${PKG_BUILD}/.${TARGET_NAME}/duckstation_libretro.so ]; then
    cp ${PKG_BUILD}/.${TARGET_NAME}/duckstation_libretro.so ${INSTALL}/usr/lib/libretro/
  else
    echo "duckstation_libretro.so not built — skipped"
  fi
}
