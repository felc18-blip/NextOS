...
pre_configure_host() {
  # GCC 15+ uses local-exec TLS model even with -fPIC at -O2, causing
  # relocation errors when linking static libs into libLLVM.so.
  # Force global-dynamic TLS model for shared library compatibility.
  export CFLAGS="${CFLAGS} -fPIC -ftls-model=global-dynamic"
  export CXXFLAGS="${CXXFLAGS//-std=gnu17/-std=gnu++17} -fPIC -ftls-model=global-dynamic"
