...
PKG_VERSION="2ec8457ab33d539b6f1fecc998360c0b8b05ed4f"
...
pre_configure_host() {
  export CXXFLAGS="${CXXFLAGS//-std=gnu17/-std=gnu++17}"
  # GCC 15/16 endurecem -Wmaybe-uninitialized / -Warray-bounds
  # SPIRV-Tools tem -Werror habilitado, então quebra build. Suprimir
  # explicitamente — não é regressão de código, é heurística do GCC.
  export CXXFLAGS+=" -Wno-error=maybe-uninitialized -Wno-error=array-bounds -Wno-error"
}

pre_configure_target() {
  export CXXFLAGS="${CXXFLAGS//-std=gnu17/-std=gnu++17}"
  export CXXFLAGS+=" -Wno-error=maybe-uninitialized -Wno-error=array-bounds -Wno-error"
  export TARGET_CXXFLAGS="${TARGET_CXXFLAGS} -Wno-error"
}
