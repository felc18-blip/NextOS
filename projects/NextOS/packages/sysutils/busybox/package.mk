...
makeinstall_target() {
  ...
  mkdir -p ${INSTALL}/usr/lib/nextos/
    cp ${PKG_DIR}/scripts/functions ${INSTALL}/usr/lib/nextos/
    cp ${PKG_DIR}/scripts/fs-resize ${INSTALL}/usr/lib/nextos/
    sed -e "s/@DISTRONAME@/${DISTRONAME}/g" \
        -i ${INSTALL}/usr/lib/nextos/fs-resize
  ...
  # Empty file - systemd generates on first boot (NOT symlink to /storage which may not be mounted)
    touch ${INSTALL}/etc/machine-id

  # /etc/mtab is needed by udisks etc...
    ln -sf /proc/self/mounts ${INSTALL}/etc/mtab

  # create /etc/hostname (real file, persists across reboots)
    echo "nextos" > ${INSTALL}/etc/hostname

  # create folder for named tables support (real dir, not symlink to /storage)
