...
post_patch() {
  ...
  if [ -d "${DTS_SOURCE_DIR}" ]; then
    ...
    # Register custom DTS files in kernel Makefile so they get compiled
    for dts_file in "${DTS_SOURCE_DIR}"/rockchip/rk3326-*.dts; do
      [ -f "${dts_file}" ] || continue
      dtb_name="$(basename "${dts_file}" .dts).dtb"
      makefile="${PKG_BUILD}/arch/arm64/boot/dts/rockchip/Makefile"
      if ! grep -q "${dtb_name}" "${makefile}"; then
        echo "dtb-\$(CONFIG_ARCH_ROCKCHIP) += ${dtb_name}" >> "${makefile}"
      fi
    done
  fi
}
...
make_target() {
  # arm64 mainline não tem target uImage.* — buildar Image e wrap via mkimage.
  if [[ "${TARGET_KERNEL_ARCH}" = "arm64" && "${KERNEL_TARGET}" = uImage* ]]; then
    if [ -z "${KERNEL_UIMAGE_LOADADDR}" -o -z "${KERNEL_UIMAGE_ENTRYADDR}" ]; then
      die "ERROR: KERNEL_UIMAGE_LOADADDR e KERNEL_UIMAGE_ENTRYADDR precisam estar setados pra uImage"
    fi
  ...
  # arm64 + uImage: wrap o Image via u-boot mkimage com compress detect
  ...
  # perf build DESABILITADO em Amlogic-nxtos: kernel 7.0.8 perf precisa
  # rustc com target aarch64-unknown-linux-gnu (tools/perf/tests/workloads/code_with_type.rs)
  # que não está no toolchain NextOS. Bloco perf inteiro removido pra desbloquear build.
}
